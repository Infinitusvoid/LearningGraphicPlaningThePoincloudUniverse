...
{
	// === Random Engine Setup ===
	// A thread-local Mersenne Twister engine, seeded once per thread.
	...
	// Sets the engine seed explicitly (for reproducibility)
	...
	inline const T& random_element(const std::vector<T>& vec)
	{
		...
		int index = random_int(0, static_cast<int>(vec.size()) - 1);
		return vec[index];
	}
	...
	// Returns random elements from `vec`.
	// If `unique` is true, elements will not repeat and at most vec.size() elements are returned.
	// If `unique` is false, exactly `count` elements are returned and duplicates are allowed.
	template<typename T>
